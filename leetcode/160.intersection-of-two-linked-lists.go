...
func getIntersectionNode(headA, headB *ListNode) *ListNode {
	skip, original := skipforward(headA, headB)

	for skip != nil && original != nil {
		if skip == original {
			return skip
		}
		skip = skip.Next
...
// skipforward returns a pruned version of the longest list such that
// it has the same length as the shorter one, and returns it together with
// the shorter one (unchanged)
func skipforward(nodeA, nodeB *ListNode) (*ListNode, *ListNode) {
	// since we only care about intersection
